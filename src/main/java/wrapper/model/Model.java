...
public class Model {
    ...
    private final HintBuffer hintBuffer = new HintBuffer();
    ...
    public Constraint addLessThanOrEqualToConstraint(final LinearExpression expression, double rhs) {
        this.state.onModelChangeRequested();
        return addConstraint(Double.NEGATIVE_INFINITY, rhs, expression, Constraint.Type.LESS_THAN_OR_EQUAL_TO);
    }
    ...
    public Constraint addLessThanOrEqualToConstraint(final LinearExpression expression, final LinearExpression rhs) {
        ...
        return addConstraint(
                ...
                completeExpression,
                Constraint.Type.LESS_THAN_OR_EQUAL_TO
        );
    }
    ...
    public Constraint addGreaterThanOrEqualToConstraint(final LinearExpression expression, double rhs) {
        this.state.onModelChangeRequested();
        return addConstraint(rhs, Double.POSITIVE_INFINITY, expression, Constraint.Type.GREATER_THAN_OR_EQUAL_TO);
    }
    ...
    public Constraint addGreaterThanOrEqualToConstraint(final LinearExpression expression, final LinearExpression rhs) {
        ...
        return addConstraint(
                ...
                completeExpression,
                Constraint.Type.GREATER_THAN_OR_EQUAL_TO
        );
    }
    ...
    void setHint(double hint, final Variable variable) {
        this.hintBuffer.setHint(hint, variable);
    }
        ...
        runHighsActionAndThrowOnError(
                () -> switch (constraint.getType()) {
                    ...
                    case GREATER_THAN_OR_EQUAL_TO ->
                            highs.changeRowBounds(constraint.getIndex(), newRhs, Double.POSITIVE_INFINITY);
                    case LESS_THAN_OR_EQUAL_TO ->
                            highs.changeRowBounds(constraint.getIndex(), Double.NEGATIVE_INFINITY, newRhs);
    ...
    private Constraint addConstraint(
        ...
        return new Constraint(this.highs.getNumRow() - 1, this, type);
    }

    private void setInitialSolution() {
        if (!this.hintBuffer.hasHints()) {
            return;
        }
        final int nmbHints = this.hintBuffer.getNmbHints();
        final VariableConsumer variableConsumer = new VariableConsumer(this, nmbHints);
        this.hintBuffer.consumeHints(variableConsumer);
        this.hintBuffer.clear();
        runHighsActionAndThrowOnError(
    ...
    @NullMarked
    @NoArgsConstructor
    private static class HintBuffer {
